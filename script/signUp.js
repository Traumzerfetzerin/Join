 ...
 * Displays specific error messages for missing or incorrect fields.
...
function validateSignUp(event) {
    ...
    // Gather errors from each validation function
    errors.push(...validateName());
    errors.push(...validateEmailField());
    errors.push(...validatePasswordField());
    errors.push(...validatePasswordMatch());

    // Display errors or proceed with sign-up
    if (errors.length > 0) {
...
/**
 * Validates the name field and returns any validation errors.
...
function validatePasswordField() {
    ...
    if (!validatePassword(password)) {
        errors.push('Password must be at least 6 characters, contain an uppercase letter and a number');
...
/**
 * Handles sign-up errors by displaying a toast message and highlighting the relevant input fields.
 * @param {string} message - The error message to display in the toast notification.
 * @param {boolean} [clearPasswords=false] - Indicates whether to clear the password fields.
 */
function handleSignUpError(message, clearPasswords = false) {
    showToast(message);
